#include "memory.h"
#include "stdint.h"
#include "print.h"

#define PG_SIZE         4096        // 4 KB 的页
#define MEM_BITMAP_BASE 0xc009a000  // 位图基地址
#define K_HEAP_START    0xc0100000  // 栈底

// 物理内存池结构
struct pool {
    struct bitmap   pool_bitmap;        
    uint32_t        phy_addr_start;
    uint32_t        pool_size;
};

struct pool             kernel_pool;
struct pool             user_pool;
struct virtual_addr     kernel_vaddr;

// 初始化内存池
static void mem_pool_init(uint32_t all_mem) {
    put_str("  mem_pool_init start\n");

    // 页目录 4KB + 768 ~ 1024 + 0 (769 和 0 指向同一个物理页) = 256
    uint32_t page_table_size = PG_SIZE << 8;
    uint32_t used_mem = page_table_size + 0x100000;     // 低端 1 MB
    uint32_t free_mem = all_mem - used_mem;
    uint16_t all_free_pages = free_mem / PG_SIZE;

    uint16_t kernel_free_pages = all_free_pages >> 1;
    uint16_t user_free_pages = all_free_pages - kernel_free_pages;

    // bitmap 中 1 bit 表示一页， / 8 用字节做单位，表示多少个位
    uint32_t kbm_length = kernel_free_pages >> 3;
    uint32_t ubm_length = user_free_pages >> 3;

    // 内核内存池起始地址
    uint32_t kp_start = used_mem;
    // 用户内存池起始地址
    uint32_t up_start = kp_start + kernel_free_pages * PG_SIZE;

    kernel_pool.phy_addr_start = kp_start;
    user_pool.phy_addr_start = up_start;

    kernel_pool.pool_size = kernel_free_pages * PG_SIZE;
    user_pool.pool_size = user_free_pages * PG_SIZE;

    kernel_pool.pool_bitmap.btmap_bytes_len = kbm_length;
    user_pool.pool_bitmap.btmap_bytes_len = ubm_length;

    // 存储位图
    kernel_pool.pool_bitmap.bits = (void*)MEM_BITMAP_BASE;
    user_pool.pool_bitmap.bits = (void*)(MEM_BITMAP_BASE + kbm_length);

    // 输出内存池信息
    put_str("    kernel_pool_bitmap_start:");
    put_int((int)kernel_pool.pool_bitmap.bits);
    put_char('\n');
    put_str("    kernel_pool_phy_addr_start:");
    put_int(kernel_pool.phy_addr_start);
    put_char('\n');

    put_str("    user_pool_bitmap_start:");
    put_int((int)user_pool.pool_bitmap.bits);
    put_char('\n');
    put_str("    user_pool_phy_addr_start:");
    put_int(user_pool.phy_addr_start);
    put_char('\n');

    // 将位图置 0
    bitmap_init(&kernel_pool.pool_bitmap);
    bitmap_init(&user_pool.pool_bitmap);

    // 初始化内核虚拟内存池的位图，按照实际物理内存大小生成数组
    kernel_vaddr.vaddr_bitmap.btmap_bytes_len = kbm_length;

    // 指向未使用的内存，目前在内核内存池和用户内存池之外
    kernel_vaddr.vaddr_bitmap.bits = \
        (void*)(MEM_BITMAP_BASE + kbm_length + ubm_length);
    kernel_vaddr.vaddr_start = K_HEAP_START;
    bitmap_init(&kernel_vaddr.vaddr_bitmap);

    put_str("  mem_pool_init done!\n");
}


// 初始化入口
void mem_init() {
    put_str("mem_init start\n");
    uint32_t mem_bytes_total = (*(uint32_t*)(0xb00));
    mem_pool_init(mem_bytes_total);
    put_str("mem_init done!\n");
}